...
from data_npy import PatchSet, get_image_pair
import utils
...
class Experiment(object):
    ...
    def train_on_epoch(self, n_epoch, train_loader):
        ...
        for idx, (inputs, target) in enumerate(train_loader):
            ...
            t_end = timer()
            print(f"Epoch: {n_epoch} {idx} / {batches}, time: {t_end-t_start}s', ssim: {score_ssim}")
    ...
    def train(self, train_dir, val_dir, patch_size, patch_stride, batch_size, num_workers=6, epochs=30, resume=True):
        # 加载数据
        ...
        train_loader = DataLoader(train_set, batch_size=batch_size, shuffle=True,num_workers=num_workers, drop_last=True, pin_memory=True, persistent_workers=True)
        val_loader = DataLoader(val_set, batch_size=batch_size, num_workers=num_workers, drop_last=True, pin_memory=True, persistent_workers=True)
    ...
    def test(self, test_dir, patch_size, num_workers=0):
        ...
        image_dirs = sorted([os.path.join(test_dir, g) for g in self.image_dirs])
        ...
        with torch.no_grad():
            ...
            for inputs, _ in test_loader:
                ...
                if len(patches) == n_blocks:
                    ...
                    save_name = image_paths[im_count][3]
                    np.save(result, os.path.join('./prediction_dcstfn', "dcstfn_%s" % save_name))
